...
/// 功耗读取器：通过 IORegistry 的 AppleSmartBattery / AppleSmartBatteryManager 服务读取功耗数据
/// 移植自 mac-scope (https://github.com/shenmuoso/mac-scope)
enum PowerMetricsReader {
    static func read() -> PowerUsage {
        let batteryProperties = properties(for: "AppleSmartBattery") ?? [:]
        let managerProperties = properties(for: "AppleSmartBatteryManager") ?? [:]
        return parse(managerProperties: managerProperties, batteryProperties: batteryProperties)
    }

    static func parse(managerProperties: [String: Any], batteryProperties: [String: Any]) -> PowerUsage {
        ...
        let batteryPowerMilliwatts = plausiblePower(number(telemetry["BatteryPower"]))
            ...
            ?? calculatedBatteryPower(from: batteryProperties)
        let systemLoadMilliwatts = nonnegative(plausiblePower(number(telemetry["SystemLoad"])))
        let adapterInputMilliwatts = nonnegative(plausiblePower(number(telemetry["SystemPowerIn"])))
